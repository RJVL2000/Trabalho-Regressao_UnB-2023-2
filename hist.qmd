...
pacman::p_load(tidyverse,readxl,GGally,knitr,MASS,xtable,regclass)
```
...
dados <- read_excel("C:/Users/User/Documents/GitHub/Trabalho-Regressao_UnB-2023-2/Dados/Dados_trabalho_20232.xlsx") 
...
xtable(cor(dados_treino)[corr_index, corr_index])
...
modelo0 <- lm(log(quant_enfermeiros) ~ idade + prob_infeccao + prop_culturas_rotina + prop_raio_x_torax_rotina + quant_leitos + filiacao_escola_medicina + regiao + media_pacientes + servicos_disponiveis, data=dadosteste)
summary(modelo0)

modelo0_stepwise <- step(modelo0, direction = "both")
summary(modelo0_stepwise)

modelo1 <- lm(log(quant_enfermeiros) ~ ., data=dadosteste)
summary(modelo1)
...
VIF(modelo0)
modelo1 <- lm(quant_enfermeiros ~ ., data = dados %>% mutate(tempo_internacao = NULL, quant_leitos = NULL))
VIF(modelo1)
